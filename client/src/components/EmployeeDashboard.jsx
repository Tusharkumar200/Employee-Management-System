...
const EmployeeDashboard = ({ data }) => {
    ...
    return (
        <div className="animation-fade-in">
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6 sm:gap-5 mb-8'>
                {cards.map((card, index) => (
                    <div key={index} className='card card-hover p-5 sm:p-6 relative overflow-hidden group flex items-center justify-between' >
                        ...
                        <card.icon className='size-10 p-2.5 rounded--lg bg-slate-100 text-slate-600 group-hoverbg-indigo-50 group-hover:text-indigo-600 transition-colors duration-200' />
                    </div>
                ))}
            </div>

            <div className='flex flex-col sm:flex-row gap-3'>
                <Link to="/attendance" className="btn btn-primary text-center inline-flex items-center justify-center gap-2">
                    Mark Attendance <ArrowRightIcon className='w-4 h-4' />
                </Link>

                <Link to="/leave" className="btn btn-secondary text-center">
                    Apply for Leave
                </Link>
            </div>
        </div>
