...
//send email to admin, if admin does't take action on leave request within 24hrs
const leaveApplicationReminder = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        if (leaveApplication?.status === "PENDING") {
            ...
            // send reminder email to admin to take action on leave request.
            await sendEmail({
        ...
        if (absentEmployees.length > 0) {
            await step.run("send-reminder-email", async () => {
                const emailPromises = absentEmployees.map(async (emp) => {
                    return sendEmail({
                        to: emp.email,
                        subject: `Application Reminder - Please Mark your Attendance`,
                        body: `<div style="max-width: 600px; font-family: Arial, sans-serif;">
                <h2>Hi ${emp.firstName},👋</h2>
                <p style="font-size: 16px;"> We notice you haven't marked your attendance for today yet. </p>
                <p style = "font-size: 16px;"> The deadline was <strong>11:30 AM </strong> and your attendance is still missing.</p>
                <p style="font-size: 16px;">Please check in as soon as possible or contact your admin if you'r facing any issues.</p>
                
                <br />
                <p style="font-size: 14px; color: #666;">Department: ${emp.department}</p>
                <p style="font-size: 16px;">Best Regards, </p>
                <p style="font-size: 16px;">QuickEMS</p>
                </div>
                `
                    })
                })
